...
class Population:
    def __init__(self, pop_size, generation=0, pop: List[Function] | Population | None = None): #pop size = 20 default
        if pop is None:
            self._population = []
    ...
    def register_function(self, func: Function):
        ...
        try:
            ...
            if len(self._next_gen_pop) >= self._pop_size or (len(self._next_gen_pop) >= self._pop_size // 4 and self._generation == 0):
                ...
                for i in range(crt_pop_size):
                    for j in range(i + 1, crt_pop_size):
                        if (np.array(pop[i].score) <= np.array(pop[j].score)).all():
                            '''
                            Exlain calc_syntax_match:
                                print(calc_syntax_match(code1, code2, 'python')), higher is more similar (max 1) 0-1
                            '''
                ...
                dominated_counts_ = dominated_counts.sum(0) # sum down the column (Ex: 3x3 -> 1x3) # the higher the score, the less dominated that code be (so the code has higher selection probabiltiy)
                print(f"Length of population in register function: {len(pop)}")

                self._population = [pop[i] for i in np.argsort(dominated_counts_)[::-1][:self._pop_size]]  # sort from worse to best
                print(f"Length of population in register function: {len(self._population)}")
                self._next_gen_pop = []
                self._generation += 1
