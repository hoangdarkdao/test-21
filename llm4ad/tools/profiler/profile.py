...
class ProfilerBase:
    ...
    def register_function(self, prompt: str, function: Function, program: str = '', *, resume_mode=False, **kwargs):
        ...
        try:
            ...
            self._num_samples += 1
            self._record_and_print_verbose(prompt, function, program, resume_mode=resume_mode)
    ...
    def _write_json(self, prompt: str, function: Function, program: str, *, record_type: Literal['history', 'best'] = 'history',
                    record_sep=300, **kwargs):
    ...
    def _record_and_print_verbose(self, prompt:str, function, program='', *, resume_mode=False):
            ...
            if score is not None:
                for i in range(self._num_objs):
                    if score[i] < self._cur_best_program_score[i]:
                        ...
                        self._write_json(prompt, function, record_type='history', program=program)
