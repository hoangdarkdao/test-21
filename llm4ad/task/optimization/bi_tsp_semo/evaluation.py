...
def evaluate(instance_data, n_instance, problem_size, ref_point, eva: callable):
    ...
    # --------- run once and collect ----------
    Archives = []
    times = []
    for _, (instance, distance_matrix_1, distance_matrix_2) in enumerate(instance_data): # just 4
        start = time.time()
    ...
    # --------- compute global z_ideal, z_nadir ----------
    all_objs_concat = np.vstack(all_objs) # just 4, not quite generalize
    ...
    # --------- compute HV with global normalization ----------
